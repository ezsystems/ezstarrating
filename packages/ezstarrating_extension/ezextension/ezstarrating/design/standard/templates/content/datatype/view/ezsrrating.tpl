...
<ul id="ezsr_rating_{$attribute.id}" class="ezsr-rating" style="display: none;" >
...
<p id="ezsr_has_rated_{$attribute.id}" class="ezsr_has_rated" style="display: none;">You have already rated this page, you can only rate once!</p>
...
YUI( YUI3_config ).use('node', 'event', 'io-ez', function( Y )
{
    ...
    {
    	Y.all('ul.ezsr-rating').setStyle('display', '');
    	Y.all('ul.ezsr-rating li a').on( 'click', _rate );
    });
